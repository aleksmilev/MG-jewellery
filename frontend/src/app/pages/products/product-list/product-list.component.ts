...
export class ProductListComponent {
	...
	constructor(
		...
		this.$product_list = this.http.Get('/items/products', {filters: JSON.stringify(this.page_filters)});
	}

	filter_check(){
		const temp_page_filters = (this.activated_route.snapshot.paramMap.get('filters') || 'gender:&max_price:&min_price:100&categorie:');
		const allowed_filters = ['gender', 'max_price', 'min_price', 'categorie'];
		...
		if(filtersMatchAllowed){
			...
			.filter(str => {
				try {
					const filters = ['gender', 'max_price', 'min_price', 'categorie'];
					...
					if (filters.includes(filter)) {
						if (value === '') {
							...
							if (!regex.test(value) || parseInt(value) <= 0) {
								this.$product_list = this.http.Get('/items/products', {filters: JSON.stringify(this.page_filters)});
								this.router.navigateByUrl(`/products/${this.page_number()}/gender:&max_price:&min_price:&categorie:`);
	...
	to_page(to_page_number: number){ 
		this.$product_list = this.http.Get('/items/products', {filters: JSON.stringify(this.page_filters)});
		this.router.navigateByUrl(`/products/${to_page_number}/${this.filter_to_url()}`) 
	}
	...
	set_gender(gender_id: number){
		...
		this.$product_list = this.http.Get('/items/products', {filters: JSON.stringify(this.page_filters)});
		this.router.navigateByUrl(`/products/${this.page_number()}/${this.filter_to_url()}`)
	}
