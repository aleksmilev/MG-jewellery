...
@Component({
    ...
    styleUrl: './client-personal-info.component.css'
})
export class ClientPersonalInfoComponent implements OnInit{
    info: any;
    ...
    ngOnInit(): void {
        this.info = this.profile.login_check();
    ...
    delete_profile(){
        this.http.Delete('/client/info', {id: this.info.id}).subscribe(data => {
            if(data.success === true){
                this.profile.logout();
